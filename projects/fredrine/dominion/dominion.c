...
int adventurerCard(struct gameState *state, int currentPlayer) {
	...
	while(drawntreasure<2){
		...
		if (cardDrawn == copper || cardDrawn == silver || cardDrawn == gold)
			drawntreasure += 2; // bug introduced here, incremented by 2 instead of 1
		else{
...
int stewardCard(struct gameState *state, int currentPlayer, int choice1, int choice2, int choice3, int handPos) {
	if (choice1 == 2) { // choice changed from 1 to 2
		//+2 cards
		drawCard(currentPlayer, state);
		drawCard(currentPlayer, state);
	} else if (choice1 == 1) { // choice changed from 2 to 1
		//+2 coins
...
int cardEffect(int card, int choice1, int choice2, int choice3, struct gameState *state, int handPos, int *bonus)
{
  ...
  switch( card ) 
    ...
    case steward:
		return stewardCard(state, currentPlayer, choice1, choice2, choice3, handPos);
